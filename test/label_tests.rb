...
class LabelsTests < Test::Unit::TestCase
  ...
  def test_object_creator
    ...
    rule = JetPEG::Compiler.compile_rule "'a' char:. 'c' <TestClassA { a: 'test1', b: [ <TestClassB 'test2'>, <TestClassB { r: @char }> ] }>"
    assert rule.parse("abc", class_scope: self.class) == TestClassA.new({ a: "test1", b: [ TestClassB.new("test2"), TestClassB.new({ r: "b" }) ] })
  end
